...
final isFavouriteProvider =
  ...
  final localStorageRepository =
      ref.watch(localStorageRepositoryProvider(movieId));

  return localStorageRepository.isMovieFavourite(movieId);
});

class _CustomSliverAppbBar extends ConsumerWidget {
  final Movie movie;
  const _CustomSliverAppbBar({required this.movie});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final AsyncValue isFavouriteFuture =
        ref.watch(isFavouriteProvider(movie.id));
    ...
    return SliverAppBar(
      ...
      actions: [
        IconButton(
            onPressed: () {
              ref
                  .read(localStorageRepositoryProvider(movie.id))
